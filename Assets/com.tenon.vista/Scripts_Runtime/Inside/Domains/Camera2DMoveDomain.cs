...
namespace TenonKit.Vista.Camera2D {
    ...
    internal static class Camera2DMoveDomain {
        ...
        internal static Vector2 GetDriverTargetWithoutDamping(Camera2DContext ctx, int id, Vector2 driverWorldPoint, float deltaTime) {
            ...
            if (softZoneDiff == Vector2.zero) {
                var deadZoneWorldDiff = Camera2DMathUtil.ScreenToWorldLength(currentCamera, deadZoneDiff, ctx.ScreenSize);
                targetPos = cameraWorldPoint + deadZoneWorldDiff;
                return targetPos;
            }
        ...
        internal static Vector2 GetDriverTargetWithDamping(Camera2DContext ctx, int id, Vector2 driverWorldPoint, float deltaTime) {
            var has = ctx.TryGetCamera(id, out var currentCamera);
            if (!has) {
                V2Log.Error($"MoveByDriver Error, Camera Not Found: ID = {id}");
                return Vector2.zero;
            ...
            if (softZoneDiff == Vector2.zero) {
                ...
                var dampingOffsetX = EasingHelper.Easing(0f,
                                                            ...
                                                            1 - dampingX,
                                                            1,
                                                            currentCamera.EasingType,
                                                            currentCamera.EasingMode);
        ...
        internal static void RefreshCameraPos(Camera2DContext ctx, int id, Vector2 cameraWorldPoint) {
            var has = ctx.TryGetCamera(id, out var currentCamera);
            if (!has) {
